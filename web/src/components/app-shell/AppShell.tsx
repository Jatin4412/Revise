...
export function AppShell() {
  ...
  return (
    <main className="relative flex h-dvh w-full overflow-hidden bg-reiterate-bg p-[clamp(.5rem,1vw,.9rem)] text-reiterate-text">
      ...
      <div className={`relative z-10 flex min-h-0 min-w-0 flex-1 overflow-hidden lg:gap-[clamp(.65rem,1vw,1rem)] lg:overflow-visible ${sidebarOpen ? "lg:rounded-none lg:border-0 lg:bg-transparent lg:shadow-none" : "rounded-[clamp(1.25rem,1.8vw,1.75rem)] border border-white/[0.07] bg-reiterate-bg/90 shadow-[0_1rem_4rem_rgba(0,0,0,.22),inset_0_1px_rgba(255,255,255,.035)] backdrop-blur-xl"}`}>
        ...
        <section className={`relative z-10 flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-[clamp(1.15rem,1.6vw,1.5rem)] bg-reiterate-bg/80 lg:bg-transparent ${!sidebarOpen ? "lg:rounded-none" : ""}`} aria-label="Conversation workspace">
