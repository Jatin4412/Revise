...
from engine.revise.models import Decision, DimensionResult, EvaluationProfile, EvaluationResult, Evidence, Issue, TaskContract, Version
...
class FakeSecondary:
    ...
    def evaluate_candidate(self, contract, response, profile):
        ...
        dimension = DimensionResult(score, 1.0, "pass")
        return EvaluationResult(Decision.ACCEPT, score, 1.0, {profile.dimensions[0]: dimension})


class PolicyStabilizationTests(unittest.TestCase):
    def test_evidence_requirement_blocks_acceptance(self):
        profile = EvaluationProfile(
            dimensions=("correctness",), required_dimensions=("correctness",), minimum_scores={"correctness": 0.7},
            evidence_requirements=("deterministic",), minimum_overall_score=0.7,
        )
        result = EvaluationResult(
            Decision.ACCEPT, 0.95, 1.0,
            {"correctness": DimensionResult(0.95, 1.0, "pass")},
            evidence=(Evidence("secondary", "llm", "pass", 1.0),),
        )
    ...
    def test_evidence_requirement_accepts_matching_pass(self):
        ...
        result = EvaluationResult(
            Decision.ACCEPT, 0.95, 1.0,
            {"correctness": DimensionResult(0.95, 1.0, "pass")},
            evidence=(Evidence("deterministic.arithmetic", "deterministic", "pass", 1.0),),
        )
    ...
    def test_hard_gate_blocks_even_when_issue_is_minor(self):
        ...
        result = EvaluationResult(
            Decision.ACCEPT, 0.95, 1.0,
            {"correctness": DimensionResult(0.95, 1.0, "pass")},
            issues=(Issue("blocked_claim", __import__("engine.revise.models", fromlist=["Severity"]).Severity.MINOR, "blocked"),),
        )
        self.assertEqual(decide(TaskContract(goal="x"), profile, result, revisions_used=0).decision, Decision.REVISE)
    ...
    def test_stop_on_no_improvement_ends_revision_loop(self):
        ...
        engine = Engine(FakePrimary(["first", "second", "third"]), secondary=FakeSecondary([0.8, 0.8, 0.8]))
    ...
    def test_best_version_excludes_regressed_revision(self):
        ...
        bad = Version("v1", "bad", bad_eval, parent_id="v0", metadata={"revision_assessment": type("Assessment", (), {"status": "regressed"})()})
