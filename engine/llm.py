...
class LLMSecondary:
    ...
    def evaluate_candidate(self, contract: TaskContract, response: str, profile: EvaluationProfile) -> EvaluationResult:
        return _parse_evaluation(self._generate_text(_evaluation_prompt(contract, response, profile)), profile)
...
def _text_callable(provider: str, model: str | None, *, role: str) -> Callable[[str], str]:
    ...
    if provider == "gemini": return _gemini_callable(model, role=role)
    if provider == "groq": return _groq_callable(model, role=role)
    if provider == "openrouter": return _openrouter_callable(model, role=role)
    if provider == "openai": return _openai_callable(model, role=role)
    if provider == "grok": return _xai_callable(model, role=role)
...
def _gemini_callable(model: str | None, *, role: str) -> Callable[[str], str]:
    ...
    if not key: raise RuntimeError("GEMINI_API_KEY is required for the Gemini provider")
    if not selected: raise RuntimeError(f"GEMINI_{role.upper()}_MODEL or an explicit model is required for the Gemini provider")
    def call(prompt: str) -> str:
        body = _post_json(f"https://generativelanguage.googleapis.com/v1beta/models/{selected}:generateContent", {"contents":[{"parts":[{"text":prompt}]}]}, {"x-goog-api-key":key}, 60.0, "Gemini")
...
def _groq_callable(model: str | None, *, role: str) -> Callable[[str], str]:
    ...
    if not key: raise RuntimeError("GROQ_API_KEY is required for the Groq provider")
...
def _openrouter_callable(model: str | None, *, role: str) -> Callable[[str], str]:
    ...
    if not key: raise RuntimeError("OPENROUTER_API_KEY is required for the OpenRouter provider")
...
def _openai_callable(model: str | None, *, role: str) -> Callable[[str], str]:
    ...
    if not key: raise RuntimeError("OPENAI_API_KEY is required for the OpenAI provider")
    if not selected: raise RuntimeError(f"OPENAI_{role.upper()}_MODEL or an explicit model is required for the OpenAI provider")
...
def _xai_callable(model: str | None, *, role: str) -> Callable[[str], str]:
    ...
    if not key: raise RuntimeError("XAI_API_KEY is required for the Grok provider")
    if not selected: raise RuntimeError(f"XAI_{role.upper()}_MODEL or an explicit model is required for the Grok provider")
...
def _responses_callable(provider: str, endpoint: str, key: str, model: str) -> Callable[[str], str]:
    def call(prompt: str) -> str:
        return _extract_output_text(_post_json(endpoint, {"model":model,"input":prompt}, {"Authorization":f"Bearer {key}"}, 60.0, provider))
    return call


def _chat_completions_callable(provider: str, endpoint: str, key: str, model: str) -> Callable[[str], str]:
    def call(prompt: str) -> str:
        body = _post_json(endpoint, {"model":model,"messages":[{"role":"user","content":prompt}],"stream":False}, {"Authorization":f"Bearer {key}","User-Agent":"ReviseEngine/0.1"}, 60.0, provider)
        choices = body.get("choices", [])
        if not isinstance(choices, list) or not choices: raise RuntimeError(f"{provider} response did not contain choices")
        message = choices[0].get("message", {}) if isinstance(choices[0], dict) else {}
        text = message.get("content") if isinstance(message, dict) else None
        if not isinstance(text, str) or not text.strip(): raise RuntimeError(f"{provider} response did not contain output text")
        return text.strip()
    return call


def _post_json(url: str, payload: dict[str, Any], headers: dict[str, str], timeout: float, provider: str) -> dict[str, Any]:
    req = request.Request(url, data=json.dumps(payload).encode("utf-8"), method="POST", headers={"Content-Type":"application/json", **headers})
    try:
        with request.urlopen(req, timeout=timeout) as response: body = json.loads(response.read().decode("utf-8"))
    except error.HTTPError as exc:
        detail = exc.read().decode("utf-8", errors="replace")
        raise RuntimeError(f"{provider} request failed ({exc.code}): {detail}") from exc
    except error.URLError as exc:
        raise RuntimeError(f"{provider} request failed: {exc.reason}") from exc
    if not isinstance(body, dict): raise RuntimeError(f"{provider} response was not a JSON object")
    return body
...
def _generation_prompt(contract: TaskContract, context: str | None) -> str:
    ...
    if contract.desired_length: sections.append(f"Length: {contract.desired_length}")
    if contract.desired_style: sections.append(f"Style: {contract.desired_style}")
    if contract.known_context: sections.append("Context:\n"+"\n".join(contract.known_context))
    if context: sections.append("Revision context:\n"+context)
    return "\n\n".join(sections)
...
def _parse_evaluation(raw: str, profile: EvaluationProfile) -> EvaluationResult:
    ...
    except json.JSONDecodeError as exc: raise RuntimeError(f"Secondary evaluator returned invalid JSON: {exc}") from exc
    if not isinstance(payload, dict) or not isinstance(payload.get("dimensions"), dict): raise RuntimeError("Secondary evaluator response must contain a dimensions object")
    ...
    for name in profile.dimensions:
        ...
        if not isinstance(item, dict):
            ...
            continue
        score, confidence, status = item.get("score"), item.get("confidence", 0.0), item.get("status")
        if not isinstance(score, (int,float)) or not isinstance(confidence, (int,float)) or status not in {"pass","partial","fail"}:
            ...
            continue
        dimensions[name] = DimensionResult(max(0.0,min(1.0,float(score))), max(0.0,min(1.0,float(confidence))), status, str(item.get("reason","")))
    ...
    if isinstance(raw_issues, list):
        for item in raw_issues:
            ...
            try: severity = Severity(item.get("severity", Severity.MINOR.value))
            except ValueError: severity = Severity.MINOR
            issues.append(Issue(str(item.get("type","evaluation")), severity, str(item.get("description","")), item.get("location") if isinstance(item.get("location"),str) else None))

    raw_revision = payload.get("revision", {})
    if isinstance(raw_revision, dict):
        raw_instructions = raw_revision.get("instructions", [])
        revision = RevisionPlan(str(raw_revision.get("strategy")) if raw_revision.get("strategy") is not None else None, tuple(str(x) for x in raw_instructions) if isinstance(raw_instructions,list) else ())
    else: revision = RevisionPlan()

    known = [d for d in dimensions.values() if d.status != "unknown"]
    weighted = [(name, dimensions[name]) for name in profile.dimensions if dimensions[name].status != "unknown" and dimensions[name].score is not None]
    ...
    return EvaluationResult(Decision.ACCEPT, overall_score, confidence, dimensions, tuple(issues), (), revision)


def _extract_output_text(payload: dict[str, Any]) -> str:
    direct = payload.get("output_text")
    if isinstance(direct, str): return direct.strip()
    chunks: list[str] = []
    for item in payload.get("output", []):
        if not isinstance(item, dict): continue
        for content in item.get("content", []):
            if isinstance(content, dict) and content.get("type") == "output_text" and isinstance(content.get("text"),str): chunks.append(content["text"])
    return "\n".join(chunks).strip()
...
def _extract_gemini_text(payload: dict[str, Any]) -> str:
    ...
    text = "\n".join(chunks).strip()
    if not text: raise RuntimeError("Gemini response did not contain output text")
    return text
