  ...
  return (
    <main className="relative flex h-dvh w-full overflow-hidden bg-reiterate-bg text-reiterate-text lg:p-[clamp(.5rem,1vw,.9rem)]">
      ...
      <div className={`relative z-10 flex min-h-0 min-w-0 flex-1 overflow-hidden bg-reiterate-bg/90 backdrop-blur-xl lg:rounded-[clamp(1.25rem,1.8vw,1.75rem)] lg:border lg:border-white/[0.07] lg:shadow-[0_1rem_4rem_rgba(0,0,0,.22),inset_0_1px_rgba(255,255,255,.035)] lg:backdrop-blur-xl lg:gap-[clamp(.65rem,1vw,1rem)] lg:overflow-visible ${sidebarOpen ? "lg:border-0 lg:bg-transparent lg:shadow-none lg:backdrop-blur-none" : ""}`}>
        ...
        {mobileSidebarOpen && (
          <>
            <button className="fixed inset-0 z-30 bg-black/45 backdrop-blur-[1px] lg:hidden" type="button" onClick={() => setMobileSidebarOpen(false)} aria-label="Close sidebar" />
            <div className="absolute inset-y-0 left-0 z-40 w-[min(18rem,calc(100vw - 1rem))] transition-transform duration-200 lg:hidden">
              <Sidebar sessions={sessions} hasMessages={messages.length > 0} onNewSession={handleNewSession} isOpen onToggle={() => setMobileSidebarOpen(false)} />
            </div>
          </>
        )}

        <section className={`relative z-10 flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden lg:bg-transparent ${sidebarOpen ? "lg:rounded-[clamp(1.25rem,1.8vw,1.75rem)] lg:border lg:border-white/[0.07] lg:shadow-[0_1rem_4rem_rgba(0,0,0,.16),inset_0_1px_rgba(255,255,255,.035)]" : ""}`} aria-label="Conversation workspace">
          <WorkspaceHeader />
