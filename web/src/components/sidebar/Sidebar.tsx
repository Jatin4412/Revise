...
export function Sidebar({ sessions, hasMessages, onNewSession, isOpen, onToggle }: SidebarProps) {
  if (!isOpen) {
    return (
      <aside className="flex h-full w-full flex-col items-center py-2">
        <button className="grid size-10 place-items-center rounded-[0.85rem] border border-reiterate-orange/20 bg-gradient-to-br from-reiterate-orange/15 to-reiterate-red/5 text-base font-bold text-reiterate-text transition hover:border-reiterate-orange/35 hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-reiterate-orange/40" type="button" onClick={onToggle} aria-label="Open sidebar" title="Open sidebar">R</button>
        <span className="my-3 h-px w-8 bg-white/[0.06]" aria-hidden="true" />
        ...
        <div className="mt-auto flex flex-col items-center gap-3"><span className="size-1.5 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,.55)]" /><button className="grid size-10 place-items-center rounded-xl text-reiterate-dim transition hover:bg-white/[0.045] hover:text-reiterate-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-reiterate-orange/40" type="button" onClick={onToggle} aria-label="Expand sidebar" title="Expand sidebar"><Icon name="chevron-right" className="size-4" /></button></div>
      </aside>
    );
  }
