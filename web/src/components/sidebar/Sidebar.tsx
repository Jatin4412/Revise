...
export function Sidebar({ sessions, hasMessages, onNewSession, isOpen, onToggle }: SidebarProps) {
  ...
  return <aside className="flex h-full min-h-0 w-full flex-col rounded-[1.2rem] border border-white/[0.07] bg-reiterate-deep/95 p-4 shadow-[0_1.5rem_4rem_rgba(0,0,0,.22)] backdrop-blur-2xl sm:p-5">
    ...
    <nav className="mt-7" aria-label="Sessions"><div className="mb-2 flex items-center justify-between px-2 text-[0.58rem] font-bold uppercase tracking-[0.2em] text-reiterate-dim"><span>Recent sessions</span><span>{sessions.length.toString().padStart(2, "0")}</span></div><div className="space-y-0.5">{sessions.map((session, index) => <button className="group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-[0.8rem] text-reiterate-muted transition hover:bg-white/[0.035] hover:text-reiterate-text focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-reiterate-orange/40" type="button" key={session}><span className="size-1.5 shrink-0 rounded-full bg-reiterate-dim/70 transition group-hover:bg-reiterate-orange" /><span className="truncate">{index === 0 && hasMessages ? "Current session" : session}</span></button>)}</div></nav>
    <div className="mt-auto border-t border-white/[0.06] pt-3"><div className="mb-1 flex items-center gap-3 rounded-xl px-3 py-2.5"><span className="size-1.5 rounded-full bg-reiterate-orange shadow-[0_0_10px_rgba(251,146,60,.55)]" /><div><p className="text-xs font-medium">System ready</p><p className="text-[0.62rem] text-reiterate-dim">Reiterate core</p></div></div><button className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-xs text-reiterate-dim transition hover:bg-white/[0.035] hover:text-reiterate-muted" type="button"><Icon name="settings" className="size-4" />Settings</button></div>
  </aside>;
}
